...
2. 点击 **上传简历 PDF / 截图**。PDF 每份最多 5 页、10MB、提取文字不超过 10 万字，通过 [Mozilla PDF.js](https://mozilla.github.io/pdf.js/getting_started/) 在浏览器本地解析，支持多页预览和文字校对。文字版默认只发送文字，不需要视觉模型；文字不足的页面会提示可能为扫描页，要求启用视觉模型并发送页面图片，避免静默漏页。文字版也可勾选图片识别。在同一个区域点击选择、拖入 PDF 或 PNG/JPEG/WebP 图片，也可粘贴截图；支持一份 PDF 与图片混合导入，PDF 页面和图片合计最多 5 页，每个文件不超过 10MB。再次选择 PDF 会替换原 PDF，保留已有图片和手动输入文字。加密 PDF 请先解除保护；失败或取消不替换已有导入草稿。截图可调整顺序；也可直接粘贴文字。
3. 识别后的内容会建立新的简历。对照 **识别原稿** 检查原图和模糊项，修改字段，再确认已校对。
   内容编辑支持在同一区块混合添加 **自由文本** 和 **经历条目**。技能、自定义区块默认使用自由文本，可按需补充标题、公司和日期等字段；区块可改名、排序，内容可增删和排序。已有简历中仅含描述的条目自动使用精简文本表单。
...
6. 每项修改都有逐字差异的原文/建议对照：未变化的文字保持普通样式，仅删除片段显示为红色删除线，新增片段显示为绿色。特别大的改写会显示完整前后文并提示未逐字标记。确认事实准确后逐条或批量采纳。**修改记录** 可撤回整笔采纳；如果字段之后被修改，将拒绝冲突撤回，避免覆盖新内容。
...
- 站点 API Key 和审批签名密钥保存在 Worker Secrets。浏览器直连的 API Key 随连接设置自动保存在当前浏览器的 localStorage，刷新或重新打开后自动恢复；在 AI 设置中点击“清除凭证”可删除保存的 Key。站点访问口令只在页面内存中，刷新后重新输入。两者均不进入简历数据库或导出备份。
