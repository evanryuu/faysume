...
for (const template of ['classic', 'modern', 'compact']) {
  ...
  test(`${template}: preview page contents match exported PDF and update after edits`, async ({
    ...
    await appearance.getByLabel('页边距').selectOption('22')
    await appearance.getByLabel('正文字号').selectOption('12')
    await appearance.getByRole('combobox', { name: '行距', exact: true }).selectOption('1.8')
    await expect(page.getByText(/分页同步更新.*页边距 22mm/)).toBeVisible()
    await page.getByRole('button', { name: '外观', exact: true }).click()
    const customized = await comparePages(page, testInfo.outputPath(`${template}-customized-preview.pdf`))
    expect(customized.flat()).toHaveLength(95)
