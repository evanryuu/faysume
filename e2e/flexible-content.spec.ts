...
test('custom sections support text and entries together with persistence and clean preview', async ({
  ...
  await page.getByLabel('区块标题', { exact: true }).fill('开源贡献')
  await page.getByLabel('区块标题', { exact: true }).blur()
  await expect(page.getByLabel('职位 / 项目 / 学位', { exact: true })).toHaveCount(0)
  ...
  await page.getByLabel('职位 / 项目 / 学位', { exact: true }).fill('维护者')
  await page.getByLabel('职位 / 项目 / 学位', { exact: true }).blur()
  await page.reload()
  await expect(page.getByRole('textbox', { name: '文本内容', exact: true })).toHaveValue(
    '维护开源工具\n参与社区文档翻译',
  )
  await expect(page.getByLabel('职位 / 项目 / 学位', { exact: true })).toHaveValue('维护者')
  ...
  await expect(page.getByLabel('职位 / 项目 / 学位', { exact: true })).toHaveCount(2)
  await page.getByLabel('职位 / 项目 / 学位', { exact: true }).nth(1).fill('社区参与')
  await page.getByLabel('职位 / 项目 / 学位', { exact: true }).nth(1).blur()
  await page.reload()
  await expect(page.getByLabel('职位 / 项目 / 学位', { exact: true }).nth(1)).toHaveValue('社区参与')
